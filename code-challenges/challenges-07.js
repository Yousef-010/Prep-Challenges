...
const objLat = (obj) => {
    ...
    let f_nameResult = arr.join(" ");

    let lName = `${obj.lastName}`

    let arrr = lName.split(" ");

    for (let i = 0; i < arrr.length; i++) {
        arrr[i] = arrr[i].charAt(0).toUpperCase() + arrr[i].slice(1);

    }
    let l_nameResult = arrr.join(" ");

    return (`my name is ${f_nameResult} ${l_nameResult} I am ${obj.age} YO, and I love ${obj.hobby}.`)
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i].yearsOfExperience > 1) {
            ...
            if (arr[i].firstName == null) {
                ...
                t_h = arr[i].tech
                array.push({ "fullName": fullName, "tech": t_h })
