...
const recursionPattern = (int1, int2) => {
    ...
    const instedFunction = (int1,int2)=>{
        arr.push(int1)
        if (int1<0) {
            arr =arr.concat(arr.slice(0,arr.length-1).reverse())
            return arr
        }
        else {
...
const filterLinks = (str) => {
    ...
    let start = str.indexOf('www') 
     let end 
    if (str.includes('org')){
          end = str.indexOf('org')-13
    }

    else if (str.includes('com')){
          end = str.indexOf('com') -13
    }

     else if (str.includes('net')){
          end = str.indexOf('net')-13
    }
...
const isPalindrome = (str) => {
    // write your code here
    let rejectedLetter = /[\W_]/g  //remove the non-alphanumeric letters 

    let getIntoLowerCase = str.toLowerCase().replace(rejectedLetter, '')  // converting the string to lower and replace the non-alphanumeric with ''

    let getStrIntoReverse = getIntoLowerCase.split('').reverse().join('') // cutting into single letter and revers it back 

    let checkBoolean = getStrIntoReverse === getIntoLowerCase // check if the string in lower case is the same string in reverse and return true of false 

    return checkBoolean // return the final boolean value ( true || false)
